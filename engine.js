...
(function () {
  ...
  function initAudio() {
    ...
    try {
      ...
      console.warn("[engine] Web Audio API init failed:", e);
    }
  }
  ...
  function setPlayback({ playing, currentTime }) {
    ...
    // Always init on first user-gesture-triggered call
    if (!audioReady) initAudio();

    // Always try to resume — browsers suspend AudioContext until gesture
    if (audioCtx) {
      audioCtx.resume().then(() => {
        console.log(
          "[engine] AudioContext state after resume:",
          audioCtx.state,
        );
      });
    }
    ...
    if (playing) {
      try {
        ...
        syncAudio
          ...
          .then(() =>
            console.log(
              "[engine] syncAudio playing, time:",
              syncAudio.currentTime,
            ),
          )
          .catch((e) => console.warn("[engine] syncAudio play failed:", e));
      } catch (e) {
        console.warn("[engine] syncAudio seek/play error:", e);
      }
    } else {
      syncAudio.pause();
    }
